...
class DDPGAgent(BaseAgent):
    def __init__(
        ...
        self.actor_local = SimpleFCNetwork(
            1234, state_size, action_size, **network_kwargs
        )
        self.actor_target = SimpleFCNetwork(
            1234, state_size, action_size, **network_kwargs
        )
        hard_update(self.actor_local, self.actor_target)

        self.critic_local = SimpleFCNetwork(
            1234,
        ...
        self.critic_target = SimpleFCNetwork(
            1234,
            (state_size + action_size),
            1,
            output_activation=lambda x: x,
    ...
    def _learn(self, gamma):
        if len(self.replay_buffer.memory) > self.replay_buffer.batch_size:
            ...
            yi = (
                ...
                + gamma
                * self.critic_target(
                    torch.cat([next_states, action_selection], 1)
                ).detach()
            )
            ...
            policy_loss = -self.critic_local(
                torch.cat([states, actor_actions], 1)
            ).mean()
