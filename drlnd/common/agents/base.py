...
class BaseAgent:
    ...
    def load_weights(self, target_directory: str) -> None:
        ...
        assert len(weights_files) == len(
            self.networks
        ), "Expected one weights file for each network."
        for filename in weights_files:
            filepath = os.path.join(target_directory, filename)
            network_name = re.compile("(.*)(_weights.pth)").match(filename).groups()[0]
